...
import { useGetTeacherVacancyRecordyByIdForAdmin } from '@/utils/hooks/tanstack/teacher/use-get-vacancy-records-by-id-for-admin';
import { TeacherVacancyRecordsForAdminQuery, VacancyDataForVacancyRecords, TeacherVacancyRecordStats } from '@/utils/types/teacher.types';
...
import AddVacancyRecordsByAdmin from './AddVacancyRecordByAdmin';
import { RecordCardByAdmin } from './RecordCardByAdmin';
        ...
        <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 px-4 pt-4 pb-3 border-b border-border">
                <div className="min-w-0">
                    <p className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground mb-0.5">Vacancy</p>
                    <h2 className="text-lg font-bold text-foreground leading-tight truncate">{v.title}</h2>
                    {v.subject && <p className="text-sm text-muted-foreground mt-0.5">{v.subject}</p>}
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-4 px-4 py-4">
                <InfoItem icon={<MapPin className="w-4 h-4" />} label="Location" value={v.location} hint={v.location_hint} />
                <InfoItem icon={<Users className="w-4 h-4" />} label="Students" value={String(v.no_of_students)} />
                <InfoItem icon={<BookOpen className="w-4 h-4" />} label="Grade" value={v.grade} />
                <InfoItem icon={<Clock className="w-4 h-4" />} label="Time" value={v.time} />
                ...
                <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-sm font-medium text-foreground truncate">{v.commission_charge}%</p>
                    </div>
                </div>
                <InfoItem icon={<Phone className="w-4 h-4" />} label="Parent Number" value={v.contact_number} />
            </div>
            ...
            <div className="px-4 pb-4">
                <div className="rounded-xl bg-muted/40 border border-border p-4 space-y-2">
                    <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="font-bold text-foreground text-base">{paidPct}%</span>
                    </div>
    ...
    return (
        <div className="relative overflow-hidden rounded-2xl border border-border bg-card px-4 py-4 shadow-sm">
            ...
            <div className="flex items-center justify-between gap-2 mb-2">
                <span className="p-2 rounded-lg" style={{ background: `${accent}18`, color: accent }}>{icon}</span>
            </div>
...
function VacancyRecordsManagementPage({ user }: { user: User }) {
    if (!user) return null;

    const params = useParams();
    const vacancyId = (params?.["vacancy-id"] ?? "") as string;
    if (!vacancyId || vacancyId.length === 0) return null;

    const teacherId = (params?.["teacher-id"] ?? "") as string;
    if (!teacherId || teacherId.length === 0) return null;

    const [open, setOpen] = useState(false);
    const [query, setQuery] = useState<TeacherVacancyRecordsForAdminQuery>({
        limit: 30,
        page: 0,
    ...
    const { data, isLoading, refetch, isError } = useGetTeacherVacancyRecordyByIdForAdmin(query);
        ...
        <div className="min-h-screen bg-gradient-to-b from-background to-muted/20">
            <div className="container mx-auto px-6 py-8 max-w-7xl">
                ...
                {isLoading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {records.map((record) => (
                            <RecordCardByAdmin key={record.id} record={record} onRefetch={refetch} />
                        ))}
                    </div>
                )}
...
export default VacancyRecordsManagementPage;