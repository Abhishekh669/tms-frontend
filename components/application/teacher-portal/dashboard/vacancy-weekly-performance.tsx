...
import {
  CheckCircle2, XCircle, ShieldCheck, Clock, AlertCircle,
  BookOpen, TrendingUp, BarChart3, ImageIcon,
} from "lucide-react";
...
function StatStrip({ progress }: { progress: VacancyWeeklyProgress }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-1.5">
      {[
        { label: "Tests",   val: `${progress.total_tests}`,           color: "text-zinc-700 dark:text-zinc-200" },
        { label: "Passed",  val: `${passed}/${progress.total_tests}`, color: "text-emerald-600 dark:text-emerald-400" },
        { label: "Avg GPA", val: progress.avg_gpa.toFixed(2),         color: "text-teal-600 dark:text-teal-400" },
      ].map((s) => (
        <div key={s.label} className="bg-zinc-50 dark:bg-zinc-800 rounded-xl p-2 text-center">
          <p className={`text-sm font-bold ${s.color}`}>{s.val}</p>
          <p className="text-[9px] text-zinc-400 dark:text-zinc-500 font-medium uppercase tracking-wide mt-0.5">
            {s.label}
          </p>
        </div>
      ))}
...
function ChartTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-700 rounded-xl shadow-md px-3 py-2 text-xs space-y-0.5">
      <p className="font-semibold text-zinc-700 dark:text-zinc-200">{d.label}</p>
      <p className="text-zinc-500 dark:text-zinc-400">
        Mark: <span className="font-bold text-zinc-700 dark:text-zinc-200">{d.mark}/{d.full}</span>
      </p>
      <p className="text-zinc-500 dark:text-zinc-400">
        GPA: <span className="font-bold text-teal-600 dark:text-teal-400">{d.gpa.toFixed(2)}</span>
      </p>
      <p className={d.passed ? "text-emerald-600 dark:text-emerald-400 font-semibold" : "text-rose-500 dark:text-rose-400 font-semibold"}>
    ...
    {points.map((p) => {
      ...
      return (
        <div key={p.label} className="flex-1 flex flex-col items-center gap-1">
          ...
          <span className="text-[9px] text-zinc-400 dark:text-zinc-500">{p.label}</span>
        </div>
      );
    })}
  </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-100 dark:border-zinc-800 p-3 space-y-1.5">
        <p className="text-[10px] font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">
          Test details
        </p>
        {tests.map((t, i) => (
          <div key={t.id} className="flex items-center gap-2 py-1.5 border-b border-zinc-50 dark:border-zinc-800 last:border-0">
            <span className="text-[10px] font-mono text-zinc-300 dark:text-zinc-600 w-5 shrink-0">T{i + 1}</span>
            <span className="text-xs font-medium text-zinc-600 dark:text-zinc-300 flex-1 truncate">{t.subject}</span>
            <span className="text-[10px] text-zinc-400 dark:text-zinc-500 shrink-0">
              {format(new Date(t.submitted_date), "dd MMM")}
            </span>
            <span className="text-[10px] font-semibold text-zinc-600 dark:text-zinc-300 shrink-0">
            ...
            {t.image_link && (
              <a href={t.image_link} target="_blank" rel="noopener noreferrer" className="shrink-0">
  ...
  if (isLoading) {
    return (
      <div className="space-y-2 pt-2">
        ...
        <div className="h-44 bg-zinc-100 dark:bg-zinc-800 rounded-xl animate-pulse" />
  ...
  if (progress.tests.length === 0) {
    return (
      <div className="flex flex-col items-center gap-1.5 py-8 text-center">
        ...
        <p className="text-xs font-medium text-zinc-400 dark:text-zinc-500">No tests yet</p>
        <p className="text-[10px] text-zinc-300 dark:text-zinc-600">Records appear once submitted</p>
      </div>
    );
  }
  ...
  return (
    <div className="px-4 space-y-2">
      ...
      {isLoading ? (
        ...
        >
          <SelectTrigger className="w-full h-10 rounded-xl border-zinc-100 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-xs font-medium text-zinc-700 dark:text-zinc-200 shadow-sm">
            <SelectValue placeholder="Select vacancy…" />
          </SelectTrigger>
          <SelectContent className="rounded-xl border-zinc-100 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-lg">
            {dropdownItems && dropdownItems.length > 0 ? (
              dropdownItems.map((item) => (
                <SelectItem key={item.id} value={item.id} className="text-xs py-2.5">
                  <span className="font-bold font-mono text-zinc-700 dark:text-zinc-200">{item.code}</span>
      ...
      {selected && (
        <div className="flex gap-1.5 flex-wrap">
          <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full capitalize bg-teal-100 text-teal-700 dark:bg-teal-950 dark:text-teal-400">
            {selected.status}
          </span>
          <span className="text-[10px] bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 font-medium px-2 py-0.5 rounded-full">
      ...
      {!selectedId ? (
        <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-100 dark:border-zinc-800 p-6 flex flex-col items-center gap-1.5 text-center">
          <BarChart3 className="h-5 w-5 text-zinc-300 dark:text-zinc-600" />
          <p className="text-xs text-zinc-400 dark:text-zinc-500">Select a vacancy to see charts</p>
        </div>
      ) : (
